...
class Contact extends Component {
    ...
    validate(firstname, lastname, telnum, email) {
        ...
        if (this.state.touched.firstname && firstname.length < 3) {
            ...
            errors.firstname = "First name must be less than 11 characters";
        }

        if (this.state.touched.lastname && lastname.length < 3)
            errors.lastname = "Last name must be more than 2 characters";
        else if (this.state.touched.lastname && lastname.length > 10)
            errors.lastname = "Last name must be less than 11 characters";

        const reg = /^\d+$/;
        if (this.state.touched.telnum && !reg.test(telnum))
            errors.telnum = "Phone number should only contain numbers";

        if (this.state.touched.email && email.split('').filter(x => x === '@').length !== 1)
            errors.email = "Email should contain exactly one @ symbol";
